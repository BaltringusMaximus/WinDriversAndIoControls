...
//unsigned char kernelMemory[8] = { 0 };
void* kernelMemory = NULL;
//unsigned long kernelMemory = 0;
...
//    CHAR outBuffer[128] = { 0 };
    ...
    OVERLAPPED overlappedKernelMemory;
    ...
    memset(&overlappedKernelMemory, 0, sizeof(OVERLAPPED));
    ...
    */
    ...
    printf("kernel memory b4 sending = 0x%p\n", kernelMemory);
    MEMORY_ENTRY* sharedMemory = NULL;
 //   memset(sharedMemory, 0, sizeof(MEMORY_ENTRY));
    status = DeviceIoControl(device, IOCTL_GET_KERNEL_BUFFER_ADDRESS, inBuffer, sizeof(inBuffer), &kernelMemory, sizeof(kernelMemory), &bytesReturned, (LPOVERLAPPED)NULL);
    unsigned long long addressFull = 0;
    printf("bytes returned = %d\n", bytesReturned);
    printf("kernel memory = 0x%p\n", kernelMemory);
    printf("kernel memory = 0x%X%X%X%X\n", ((unsigned char*)kernelMemory)[0], ((unsigned char*)kernelMemory)[1], ((unsigned char*)kernelMemory)[2], ((unsigned char*)kernelMemory)[3]);
    //printf("kernel memory = 0x%llX\n", ((unsigned char*)sharedMemory->pBuffer)[0]);
    /*
    printf("kernel memory = ");
    
    for (int i = sizeof(kernelMemory)-1; i >= 0; i--)
    {
        printf("%02X ", kernelMemory[i]);
        addressFull = addressFull + kernelMemory[i] * pow((double)256, (double)i);
        printf("full address = %llX\n", addressFull);
    }
    printf("\n");
    printf("full address = %llX\n", addressFull);
    unsigned char* tempAddress = (unsigned char*)addressFull;
    printf("address = %p\n", tempAddress);
    printf("value at address = %llX\n", *tempAddress);
    */

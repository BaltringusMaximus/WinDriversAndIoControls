...
)
{
    ...
    status = WdfMemoryCreate(WDF_NO_OBJECT_ATTRIBUTES, NonPagedPool, WDF_MEMORY_POOL_TAG, /*393216000*/ 500000, &contiguousMemoryForPacket, &pBufferContiguousMemory);
    ...
    DbgPrint("INFO: buffer value = 0x%X,0x%X", pBufferContiguousMemoryByte, *(&pBufferContiguousMemoryByte));
    /*
...
*/
    ...
    */
    ...
    status = WdfMemoryCopyFromBuffer(contiguousMemoryForPacket, 0, valueMDL, ((int)icmpLength + 34));
    if (!NT_SUCCESS(status))
    {
